...
rawDataFolder <- "Z:/Data/Sergey Ananyev/Assay Scripts/CMV assays/Bioplex BAMA/Primary QC Script/Test Environment/2.1.1 Testing/Raw Data/"

workingDirectory <- "Z:/Data/Sergey Ananyev/Assay Scripts/CMV assays/Bioplex BAMA/Primary QC Script/Test Environment/2.1.1 Testing/"
...
```{r}
interpolateAndProcessData <- function(rawData, assayName, headerrows, blankBeadCutoff, standardCurve, dilutionfactor, workingDirectory, dilutioncurvepath, sampledilutionpath, allData, SamplingErrorsProcessed, BeadCountProcessed, CVProcessed, BlankWellProcessed, BlankBeadProcessed, currentIteration){
  ...
  qcpath <- paste(workingDirectory, "QC Files/", assayName, "_QC.csv", sep = "")
  prismpath <- paste(workingDirectory,"Prism Files/", assayName,"_PRISM.pzfx", sep = "")
  mfirawpath <- paste(workingDirectory, "Analyzed Files/Filtered MFIs/", assayName, "_MFIs.csv", sep = "")
  noninterpolatedpath <- paste(workingDirectory, "Analyzed Files/Non Interpolated/", assayName, "_OUTLIERS.csv", sep = "")
  interpolatedconcpath <- paste(workingDirectory, "Analyzed Files/Interpolated Unfiltered/", assayName, "_INTERPLTDL_CONC.csv", sep = "")
  mfirawpath_unfiltered <- paste(workingDirectory, "Analyzed Files/Raw MFIs/", assayName, "_MFIs-unfiltered.csv", sep = "")
  interpolated_filtered <- paste(workingDirectory, "Analyzed Files/Interpolated Filtered/", assayName, "_INTERPLTDL_CONC-filtered.csv", sep = "")  
...
if (nrow(mfiSTD) == 0) {
  ...
  # Filter out rows from mfisRaw where column 1 matches QC_filter_values
mfisFiltered <- mfisRaw[!mfisRaw[,1] %in% QC_filter_values,]
...
# Apply the function to all data
QCoutput <- mapply(function(x, name) interpolateAndProcessData(x, name, headerrows, blankBeadCutoff, standardCurve, dilutionfactor, workingDirectory, dilutioncurvepath, sampledilutionpath, allData, allData.SamplingErrorsProcessed, allData.BeadCountProcessed, allData.CVProcessed, allData.BlankWellProcessed, allData.BlankBeadProcessed, currentIteration), allData, names(allData), SIMPLIFY=FALSE)
```
